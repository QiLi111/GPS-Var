...
def test(test_loader,args,device,folder):
    ...
    with torch.no_grad():
        ...
        for ii, (image_val, mask_all_val, mask_wo_noise, selected_observation) in enumerate(test_loader):
            ...
            logits,var = torch.chunk(model(images_val[:,None,...]),2,dim=1)
            ...
            if args.pre_labels == 'co' or args.pre_labels == 'seg_simulated':
                ...
                raise ValueError("co and seg_simulated are not supported")
            
            elif args.pre_labels == 'random3':

                preds_val = unet_linear_without_noise(logits,var,Softplus)
                metrics_vote,index_non_empty_vote = get_metrics_all_unet(preds_val,mask_wo_noise,criterion,args,metrics_vote,index_non_empty_vote)
                
                selected_observation = 0*torch.ones(images_val.shape[0]).to(device).long()
                preds_val_0 = unet_linear_add_noise(logits,model,selected_observation,var,Softplus)
                metrics_0,index_non_empty_0 = get_metrics_all_unet(preds_val_0,mask_wo_noise,criterion,args,metrics_0,index_non_empty_0)
                
                selected_observation = 1*torch.ones(images_val.shape[0]).to(device).long()
                preds_val_1 = unet_linear_add_noise(logits,model,selected_observation,var,Softplus)
                metrics_1,index_non_empty_1 = get_metrics_all_unet(preds_val_1,mask_wo_noise,criterion,args,metrics_1,index_non_empty_1)
                
                selected_observation = 2*torch.ones(images_val.shape[0]).to(device).long()
                preds_val_2 = unet_linear_add_noise(logits,model,selected_observation,var,Softplus)
                metrics_2,index_non_empty_2 = get_metrics_all_unet(preds_val_2,mask_wo_noise,criterion,args,metrics_2,index_non_empty_2)
                ...
                raise ValueError("seg1 is not supported")
                ...
                raise ValueError("seg2 is not supported")
                ...
                raise ValueError("seg3 is not supported")
        ...
        if args.pre_labels == 'co' or args.pre_labels == 'seg_simulated':     
            ...
            hyperpara_all = {}
            hyperpara_all['noise0'] = model.transform_sigma[0].item()
            hyperpara_all['noise1'] = model.transform_sigma[1].item()
            hyperpara_all['noise2'] = model.transform_sigma[2].item()

            hyperpara_all['mu0'] = model.transform_mu[0].item()
            hyperpara_all['mu1'] = model.transform_mu[1].item()
            hyperpara_all['mu2'] = model.transform_mu[2].item()

            
            return metrics_all,hyperpara_all,saved_model_name
            ...
            # metrics_vote = list_to_numpy_unet(metrics_vote,index_non_empty_vote)
            # metrics_1 = list_to_numpy_unet(metrics_1,index_non_empty_1)
            # metrics_all = {'vote':metrics_vote,'1':metrics_1}
...
models_folders = 'previous_trained_models_UnetLinear_20250221_test_on_corrected_labels'
...
for folder in all_test_folders:
    ...
    # try:
    if True:
        metrics,hyperpara_all,saved_model_name = test(test_loader,args,device,models_folders+'/'+folder)
        # save metrics and hyperparameters
        save_metrics_linear_unet(os.getcwd()+'/'+ fd_name_save +'/'+csv_name,folder,metrics,hyperpara_all,saved_model_name,args)
